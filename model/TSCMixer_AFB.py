...
class Model(nn.Module):
    ...
    def _build_model(self):
        ...
        self.classifier = ClassificationHead(self.Dicp * 4 + self.d * 2, self.num_classes).to(self.device)
    ...
    def forward(self, x):
        ...
        X_RA_Mb_low = X_RA_low + self.Mambas[0](X_RA_low + self.H2L(hidden_states[1]))      # Adaptive frequency brigdes
        # X_RA_Mb_low = X_RA_low + self.Mambas[0](X_RA_low)                                 # No
        # X_RA_Mb_low = X_RA_low + self.Mambas[0](X_RA_low + X_RA_high)                     # Addition
        ...
        X_RA_Mb_high = X_RA_high + self.Mambas[1](X_RA_high + self.L2H(hidden_states[0]))   # Adaptive frequency brigdes
        # X_RA_Mb_high = X_RA_high + self.Mambas[1](X_RA_high)                              # No
        # X_RA_Mb_high = X_RA_high + self.Mambas[1](X_RA_high + X_RA_low)                   # Addition
        ...
        H_fusion = torch.concat([H_TFL, H_TFH, H_TMS], dim=1)

        # heatmap(pd.DataFrame(hidden_states[0][0].cpu().detach().numpy().T), f'./vis/L.jpg')
        # heatmap(pd.DataFrame(hidden_states[1][0].cpu().detach().numpy().T), f'./vis/H.jpg')
