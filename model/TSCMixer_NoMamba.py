...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        X_RA_AFB_low = self.H2L(X_RA_high) + X_RA_low
        X_RA_CA_low = X_RA_AFB_low * self.Column_wise_Attention(X_RA_AFB_low)
        H_TFL = torch.mean(X_RA_CA_low, dim=1)  # GAP:global average pooling

        X_RA_AFB_high = self.L2H(X_RA_low) + X_RA_high
        X_RA_CA_high = X_RA_AFB_high * self.Column_wise_Attention(X_RA_AFB_high)
        H_TFH = torch.mean(X_RA_CA_high, dim=1)  # GAP:global average pooling
