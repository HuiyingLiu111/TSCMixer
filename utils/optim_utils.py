...
def get_lr_scheduler(optimizer, total_steps, cfgs):
    if cfgs['training']['lr_schedule'] == 'warmup_cosine':
        from utils.lr_scheduler import LinearWarmupCosineAnnealingLR
        scheduler = LinearWarmupCosineAnnealingLR(optimizer,
                                                  warmup_epochs=cfgs['training']['warmup_steps'],
                                                  max_epochs=total_steps)
    elif cfgs['training']['lr_schedule'] == 'cosine_anneal':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_steps)
        # if args.checkpoint is not None:
        #     scheduler.step(epoch=start_epoch)
    elif cfgs['training']['lr_schedule'] == 'poly':
        def lambdas(epoch):
